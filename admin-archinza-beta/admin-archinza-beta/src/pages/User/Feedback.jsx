...
const Feedback = () => {
  ...
  const fetchFeedbacks = async () => {
    ...
    try {
      if (isSuperAdmin(userRole.role)) {
        const { data } = await http.get(feedbackUrl + "/personal");
        setFeedbacks(data?.data || []);
        setTopicOptions(data?.topics?.map((it) => it.topic) || []);
      } else if (hasPermission(userRole.role, "feeedback_assignee_personal")) {
        const { data } = await http.get(
          feedbackUrl + `/personal/${userRole?.id}`
        );
        if (data) {
  ...
  const fetchAdmins = async (role) => {
    setLoading(true);
    const { data } = await http.get(
      `${feedbackUrl}/feedback-admins/User Admin - Personal`
    );
    setUsers(data);
    setLoading(false);
  };
      ...
      render: (_, record) => (
        <Space>
          ...
          <Button
            ...
            title="Update Status"
            disabled={
              hasPermission(userRole.role, "feeedback_personal") ||
              hasExactPermission(userRole.role, "feeedback_assignee_personal")
                ? false
                : true
            }
          >
              ...
              <Row gutter={16} style={{ marginBottom: 16 }}>
                ...
                <Col span={4}>
                  ...
                  >
                    ...
                    hasPermission(userRole.role, "feeedback_assignee_personal")
