...
const FEStep02LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      e_additional_services: Joi.array().min(1).label("Support").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { e_additional_services: selectedOptions };
    ...
    let data = await http.post(
      ...
      {
        e_additional_services: selectedOptions,
        status: saveStatus,
      }
    );
  ...
  const fetchServices = async () => {
    ...
    if (data) {
      ...
      const updatedOption = data?.addon_services?.map((option) => {
        ...
        if (
          BusinessAccountContext?.data?.e_additional_services?.includes(
            option.value
          )
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.e_additional_services);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((4 / 9) * 100);
    }
  }, [isActive]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="e_additional_services_error"></div>
        {formError?.e_additional_services && (
          <p className="error">{formError?.e_additional_services}</p>
        )}
      </div>
