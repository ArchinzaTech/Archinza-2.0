...
const FEStep04LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      e_project_typology: Joi.array().min(1).label("Typology").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { e_project_typology: selectedOptions };
    ...
    let data = await http.post(
      ...
      {
        e_project_typology: selectedOptions,
        status: saveStatus,
      }
    );
  ...
  const fetchServices = async () => {
    ...
    if (data) {
      const updatedOption = data?.typologies?.map((option) => {
        if (
          BusinessAccountContext?.data?.e_project_typology?.includes(
            option.value
          )
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.e_project_typology);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((6 / 9) * 100);
    }
  }, [isActive]);
  useEffect(() => {
    fetchServices();
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="e_project_typology_error">
          <p className="error">{formError?.e_project_typology}</p>
        </div>
      </div>
