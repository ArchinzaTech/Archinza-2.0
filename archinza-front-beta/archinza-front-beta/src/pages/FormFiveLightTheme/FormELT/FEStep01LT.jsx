...
const FEStep01LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      e_product_categories: Joi.array()
        .min(1)
        .required()
        .label("Product/Material's Category")
        .messages({
          "array.min": "Please Select atleast 1 Category",
        }),
    };
  ...
  const handleSubmit = async () => {
    let form_values = { e_product_categories: selectedOptions };
  ...
  const fetchData = async () => {
    ...
    if (data) {
      ...
      const updatedOption = data?.product_categories?.map((option) => {
        option.label = capitalizeFirstLetter(option.label);
        option.value = capitalizeFirstLetter(option.value);
        if (
          BusinessAccountContext?.data?.e_product_categories?.includes(
            option.value
          )
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.e_product_categories);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((3 / 9) * 100);
    }
  }, [isActive]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="e_product_categories_error">
          {formError?.e_product_categories && (
            <p className="error">{formError?.e_product_categories}</p>
          )}
        </div>
      </div>
