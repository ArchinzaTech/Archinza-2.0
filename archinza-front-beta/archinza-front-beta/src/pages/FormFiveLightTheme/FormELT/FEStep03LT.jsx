...
const FEStep03LT = ({
  ...
  const formALocationList = locations.map((option) => (
    <React.Fragment key={option}>
      <RadioButton
        lightTheme
        label={option}
        labelId={option + "formELocation"}
        name="project_location_formE"
        checked={values.e_project_location === option}
  ...
  useEffect(() => {
    setValues({
      e_project_location: BusinessAccountContext?.data?.e_project_location,
    });
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((5 / 9) * 100);
    }
  }, [isActive]);
  useEffect(() => {
    fetchData();
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="e_project_location_error">
          <p className={`${style.top_error_with_space} ${style.error}`}>
            {formError.e_project_location}
          </p>
        </div>
      </div>
