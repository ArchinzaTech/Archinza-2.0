...
const FDStep07LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      d_design_style: Joi.array().min(1).label("Design").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { d_design_style: selectedOptions };
    ...
    let data = await http.post(
      ...
      {
        d_design_style: selectedOptions,
        status: saveStatus,
      }
    );
  ...
  const fetchServices = async () => {
    ...
    if (data) {
      const updatedOption = data?.design_styles?.map((option) => {
        if (
          BusinessAccountContext?.data?.d_design_style?.includes(option.value)
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.d_design_style);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((9 / 11) * 100);
    }
  }, [isActive]);
  useEffect(() => {
    fetchServices();
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="d_design_style_error">
          <p className="error">{formError?.d_design_style}</p>
        </div>
      </div>
