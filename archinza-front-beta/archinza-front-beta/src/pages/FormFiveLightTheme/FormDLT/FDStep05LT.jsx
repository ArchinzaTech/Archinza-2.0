...
const FDStep05LT = ({
  ...
  const formALocationList = locations.map((option) => (
    <React.Fragment key={option}>
      <RadioButton
        lightTheme
        label={option}
        labelId={option + "formDLocation"}
        name="project_location_formD"
        checked={values.d_project_location === option}
  ...
  useEffect(() => {
    setValues({
      d_project_location: BusinessAccountContext?.data?.d_project_location,
    });
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((7 / 11) * 100);
    }
  }, [isActive]);
  useEffect(() => {
    fetchData();
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="d_project_location_error">
          <p className={`${style.top_error_with_space} ${style.error}`}>
            {formError.d_project_location}
          </p>
        </div>
      </div>
