...
const FDStep01LT = ({
  ...
  const handleSubmit = async () => {
    let form_values = { d_product_categories: selectedOptions };
  ...
  const fetchData = async () => {
    ...
    if (data) {
      ...
      const updatedOption = data?.product_categories?.map((option) => {
        ...
        if (
          BusinessAccountContext?.data?.d_product_categories?.includes(
            option.value
          )
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.d_product_categories);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((3 / 11) * 100);
    }
  }, [isActive]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="d_product_categories_error">
          {formError?.d_product_categories && (
            <p className="error">{formError?.d_product_categories}</p>
          )}
        </div>
      </div>
