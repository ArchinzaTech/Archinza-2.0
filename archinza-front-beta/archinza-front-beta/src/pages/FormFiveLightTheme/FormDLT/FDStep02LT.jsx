...
const FDStep02LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      d_additional_services: Joi.array().min(1).label("Support").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { d_additional_services: selectedOptions };
    ...
    if (BusinessAccountContext?.data?.status) {
    ...
    let data = await http.post(
      ...
      {
        d_additional_services: selectedOptions,
        status: saveStatus,
      }
    );
  ...
  const fetchServices = async () => {
    ...
    if (data) {
      ...
      const updatedOption = data?.addon_services?.map((option) => {
        ...
        if (
          BusinessAccountContext?.data?.d_additional_services?.includes(
            option.value
          )
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.d_additional_services);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((4 / 11) * 100);
    }
  }, [isActive]);
    ...
    <>
      <div className={style.text_container}>
        <h1 className={style.title}>
          What services does the business provide in{" "}
          <br className={style.desktop_break} />
          addition to retail/ execution/ manufacturing?*
        </h1>
        <p className={style.description}>Choose As Many</p>
        <div id="d_additional_services_error"></div>
        {formError?.d_additional_services && (
          <p className="error">{formError?.d_additional_services}</p>
        )}
      </div>
