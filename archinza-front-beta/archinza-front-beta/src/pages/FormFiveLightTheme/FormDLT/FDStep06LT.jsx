...
const FDStep06LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      d_project_typology: Joi.array().min(1).label("Typology").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { d_project_typology: selectedOptions };
    ...
    let data = await http.post(
      ...
      {
        d_project_typology: selectedOptions,
        status: saveStatus,
      }
    );
  ...
  const fetchServices = async () => {
    ...
    if (data) {
      const updatedOption = data?.typologies?.map((option) => {
        if (
          BusinessAccountContext?.data?.d_project_typology?.includes(
            option.value
          )
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.d_project_typology);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((8 / 11) * 100);
    }
  }, [isActive]);
  useEffect(() => {
    fetchServices();
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="d_project_typology_error">
          <p className="error">{formError?.d_project_typology}</p>
        </div>
      </div>
