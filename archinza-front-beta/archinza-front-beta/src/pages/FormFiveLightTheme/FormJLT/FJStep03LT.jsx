...
const FJStep03LT = ({
  ...
  const handleSubmit = async () => {
    if (date && date?.$d == "Invalid Date") {
      setFormError({
        j_establishment_year: "Invalid Date",
      });
      helper.scroll(
    ...
    values.j_establishment_year = newDate;
  ...
  useEffect(() => {
    setDate(
      BusinessAccountContext?.data?.j_establishment_year !== ""
        ? dayjs(BusinessAccountContext?.data?.j_establishment_year)
        : ""
    );
    setValues(() => ({
      j_establishment_year:
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="j_establishment_year_error">
          <p className={`${style.rstep02Error} ${style.error}`}>
            {formError.j_establishment_year}
          </p>
        </div>
      </div>
