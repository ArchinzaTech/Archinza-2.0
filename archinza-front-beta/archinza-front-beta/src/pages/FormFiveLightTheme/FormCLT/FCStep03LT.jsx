...
const FCStep03LT = ({
  ...
  const employeeCountList = employees.map((option) => (
    <React.Fragment key={option}>
      <RadioButton
        lightTheme
        label={option}
        labelId={option + "formC"}
        name="employeeCountFormC"
        checked={values.c_employees_range === option}
  ...
  useEffect(() => {
    setValues({
      c_employees_range: BusinessAccountContext?.data?.c_employees_range,
    });
  }, [currentStep]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="c_employees_range_error">
          {formError.c_employees_range && (
