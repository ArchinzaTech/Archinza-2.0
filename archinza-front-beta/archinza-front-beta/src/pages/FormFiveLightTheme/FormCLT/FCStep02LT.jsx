...
const FCStep02LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      c_additional_services: Joi.array().min(1).label("Support").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { c_additional_services: selectedOptions };
    ...
    let data = await http.post(
      ...
      {
        c_additional_services: selectedOptions,
        status: saveStatus,
      }
    );
  ...
  const fetchServices = async () => {
    ...
    if (data) {
      ...
      const updatedOption = data?.addon_services?.map((option) => {
        ...
        if (
          BusinessAccountContext?.data?.c_additional_services?.includes(
            option.value
          )
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.c_additional_services);
  }, [currentStep]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="c_additional_services_error"></div>
        {formError?.c_additional_services && (
          <p className="error">{formError?.c_additional_services}</p>
        )}
      </div>
