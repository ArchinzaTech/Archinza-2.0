...
const FCStep07LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      c_project_typology: Joi.array().min(1).label("Typology").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { c_project_typology: selectedOptions };
    ...
    let data = await http.post(
      ...
      {
        c_project_typology: selectedOptions,
        status: saveStatus,
      }
    );
  ...
  const fetchServices = async () => {
    ...
    if (data) {
      const updatedOption = data?.typologies?.map((option) => {
        if (
          BusinessAccountContext?.data?.c_project_typology?.includes(
            option.value
          )
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.c_project_typology);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((9 / 15) * 100);
    }
  }, [isActive]);
  useEffect(() => {
    fetchServices();
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="c_project_typology_error">
          <p className="error">{formError?.c_project_typology}</p>
        </div>
      </div>
