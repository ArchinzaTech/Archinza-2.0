...
const FCStep01LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      c_services: Joi.array().min(1).required().label("Services").messages({
        "array.min": "Please Select atleast 1 Service",
      }),
    };
  ...
  const handleSubmit = async () => {
    let form_values = { c_services: selectedOptions };
  ...
  const fetchData = async () => {
    ...
    if (data) {
      const updatedOption = data?.services?.map((option) => {
        if (BusinessAccountContext?.data?.c_services?.includes(option.value)) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.c_services);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((3 / 15) * 100);
    }
  }, [isActive]);
  ...
  return (
    <>
      <div className={style.text_container}>
        <h1 className={style.title}>What services do you provide?*</h1>
        <p className={style.description}>Choose As Many</p>
        <div id="c_services_error">
          {formError?.c_services && (
            <p className="error">{formError?.c_services}</p>
          )}
        </div>
      </div>
