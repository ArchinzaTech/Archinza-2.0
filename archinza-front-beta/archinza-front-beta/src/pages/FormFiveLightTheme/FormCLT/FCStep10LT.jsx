...
const FCStep10LT = ({
  ...
  const handleSubmit = async () => {
    ...
    let errors = await validate({ c_project_scope: options });
    setFormError(errors);
  ...
  const fetchData = async () => {
    ...
    if (data) {
      ...
      if (
        ...
        BusinessAccountContext?.data?.c_project_scope?.length
      ) {
        setSelectedOptions(["ALL"]);
        setAllSelected(true);
        setAllOptions(updatedOption);
      } else {
        setSelectedOptions(BusinessAccountContext?.data?.c_project_scope);
      }
  ...
  const projectSizeList = projetScopeData.map((option) => (
    <React.Fragment key={option}>
      <CheckboxButton
        lightTheme
        label={option}
        labelId={option + "formC_project_scope"}
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="c_project_scope_error">
          <p className={`${style.rstep02Error} ${style.error}`}>
            {formError.c_project_scope}
          </p>
        </div>
