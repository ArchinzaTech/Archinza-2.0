...
const FCStep09LT = ({
  ...
  const fetchData = async (selectedCurrency) => {
    ...
    if (selectedCurrency == "INR") {
      ...
      data = await http.get(
        config.api_url +
          `/business/get-currency?base=INR&target=${selectedCurrency}&section=budget&business_type=business_type_c`
      );
      if (data) {
  ...
  useEffect(() => {
    setSelectedBudget(
      BusinessAccountContext?.data?.c_avg_project_budget?.budget
    );
    setSelectedCurrency(
      BusinessAccountContext?.data?.c_avg_project_budget?.currency ||
        selectedCurrency
    );
  }, [currentStep]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="c_avg_project_budget_error">
          <p className={`${style.top_error_with_space} ${style.error}`}>
            {formError.c_avg_project_budget}
          </p>
        </div>
      </div>
