...
const FCStep04LT = ({
  ...
  const handleSubmit = async () => {
    if (date && date?.$d == "Invalid Date") {
      setFormError({
        c_establishment_year: "Invalid Date",
      });
      helper.scroll(
    ...
    values.c_establishment_year = newDate;
  ...
  useEffect(() => {
    setDate(
      BusinessAccountContext?.data?.c_establishment_year !== ""
        ? dayjs(BusinessAccountContext?.data?.c_establishment_year)
        : ""
    );
    setValues(() => ({
      c_establishment_year:
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="c_establishment_year_error">
          <p className={`${style.rstep02Error} ${style.error}`}>
            {formError.c_establishment_year}
          </p>
        </div>
      </div>
