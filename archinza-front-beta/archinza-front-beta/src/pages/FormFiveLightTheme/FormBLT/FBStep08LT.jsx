...
const FBStep08LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      b_design_style: Joi.array().min(1).label("Design").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { b_design_style: selectedOptions };
    ...
    let data = await http.post(
      ...
      {
        b_design_style: selectedOptions,
        status: saveStatus,
      }
    );
  ...
  const fetchServices = async () => {
    ...
    if (data) {
      const updatedOption = data?.design_styles?.map((option) => {
        if (
          BusinessAccountContext?.data?.b_design_style?.includes(option.value)
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.b_design_style);
  }, [currentStep]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="b_design_style_error">
          <p className="error">{formError?.b_design_style}</p>
        </div>
      </div>
