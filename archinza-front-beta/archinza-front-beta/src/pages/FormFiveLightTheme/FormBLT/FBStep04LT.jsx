...
const FBStep04LT = ({
  ...
  const handleSubmit = async () => {
    if (date && date?.$d == "Invalid Date") {
      setFormError({
        b_establishment_year: "Invalid Date",
      });
      helper.scroll(
    ...
    values.b_establishment_year = newDate;
  ...
  useEffect(() => {
    setDate(
      BusinessAccountContext?.data?.b_establishment_year !== ""
        ? dayjs(BusinessAccountContext?.data?.b_establishment_year)
        : ""
    );
    setValues(() => ({
      b_establishment_year:
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="b_establishment_year_error">
          <p className={`${style.rstep02Error} ${style.error}`}>
            {formError.b_establishment_year}
          </p>
        </div>
      </div>
