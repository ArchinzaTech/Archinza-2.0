...
const FBStep07LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      b_project_typology: Joi.array().min(1).label("Typology").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { b_project_typology: selectedOptions };
    ...
    let data = await http.post(
      ...
      {
        b_project_typology: selectedOptions,
        status: saveStatus,
      }
    );
  ...
  const fetchServices = async () => {
    ...
    if (data) {
      const updatedOption = data?.typologies?.map((option) => {
        if (
          BusinessAccountContext?.data?.b_project_typology?.includes(
            option.value
          )
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.b_project_typology);
  }, [currentStep]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="b_project_typology_error">
          <p className="error">{formError?.b_project_typology}</p>
        </div>
      </div>
