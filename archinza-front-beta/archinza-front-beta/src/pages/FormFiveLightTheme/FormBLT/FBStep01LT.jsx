...
const FBStep01LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      b_services: Joi.array().min(1).required().label("Services").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { b_services: selectedOptions };
  ...
  const fetchData = async () => {
    ...
    if (data) {
      const updatedOption = data?.services?.map((option) => {
        if (BusinessAccountContext?.data?.b_services?.includes(option.value)) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.b_services);
  }, [currentStep]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="b_services_error">
          {formError?.b_services && (
            <p className="error">{formError?.b_services}</p>
          )}
        </div>
      </div>
