...
const FBStep05LT = ({
  ...
  const handleSubmit = async () => {
    ...
    let errors = await validate({ b_project_sizes: options });
    setFormError(errors);
  ...
  const fetchData = async () => {
    ...
    if (data) {
      ...
      if (
        ...
        BusinessAccountContext?.data?.b_project_sizes?.length
      ) {
        setSelectedOptions(["ALL"]);
        setAllSelected(true);
        setAllOptions(updatedOption);
      } else {
        setSelectedOptions(BusinessAccountContext?.data?.b_project_sizes);
      }
  ...
  const projectSizeList = projectSizes.map((option) => (
    <React.Fragment key={option}>
      <CheckboxButton
        lightTheme
        label={option}
        labelId={option + "formB_project_size"}
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="b_project_sizes_error">
          <p className={`${style.rstep02Error} ${style.error}`}>
            {formError.b_project_sizes}
          </p>
        </div>
