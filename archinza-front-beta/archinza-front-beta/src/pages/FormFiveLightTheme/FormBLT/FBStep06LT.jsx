...
const FBStep06LT = ({
  ...
  const formALocationList = locations.map((option) => (
    <React.Fragment key={option}>
      <RadioButton
        lightTheme
        label={option}
        labelId={option + "formBLocation"}
        name="project_location_formB"
        checked={values.b_project_location === option}
  ...
  useEffect(() => {
    setValues({
      b_project_location: BusinessAccountContext?.data?.b_project_location,
    });
  }, [currentStep]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="b_project_location_error">
          <p className={`${style.top_error_with_space} ${style.error}`}>
            {formError.b_project_location}
          </p>
        </div>
      </div>
