...
const FGStep01LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      g_services: Joi.array().min(1).required().label("Services").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { g_services: selectedOptions };
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.g_services);
  }, [currentStep]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="g_services_error">
          {formError?.g_services && (
            <p className="error">{formError?.g_services}</p>
          )}
        </div>
      </div>
