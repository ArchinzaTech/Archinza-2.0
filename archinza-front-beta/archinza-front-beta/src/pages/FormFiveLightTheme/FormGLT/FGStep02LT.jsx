...
const FGStep02LT = ({
  ...
  const handleSubmit = async () => {
    if (date && date?.$d == "Invalid Date") {
      setFormError({
        g_establishment_year: "Invalid Date",
      });
      helper.scroll(
    ...
    values.g_establishment_year = newDate;
  ...
  useEffect(() => {
    setDate(
      BusinessAccountContext?.data?.g_establishment_year !== ""
        ? dayjs(BusinessAccountContext?.data?.g_establishment_year)
        : ""
    );
    setValues(() => ({
      g_establishment_year:
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="g_establishment_year_error">
          <p className={`${style.rstep02Error} ${style.error}`}>
            {formError.g_establishment_year}
          </p>
        </div>
      </div>
