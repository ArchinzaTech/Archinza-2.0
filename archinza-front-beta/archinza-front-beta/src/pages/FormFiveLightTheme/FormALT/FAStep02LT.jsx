...
const FAStep02LT = ({
  ...
  const handleSubmit = async () => {
    ...
    let nextStepToGo = currentStep + 2;
    let saveStatus = nextStepToGo;
    ...
    if (data) {
      ...
      goToStep(nextStepToGo);
      window.scrollTo(0, 0);
    }
  };

  const fetchData = async () => {
    if (BusinessAccountContext?.data?.services) {
      const modifiedOptions = BusinessAccountContext.data.services.map(
        (option) => ({
          label: option,
          value: option,
        })
  ...
  return (
    <>
      ...
      <div className={style.text_container}>
        ...
        <h1 className={style.title}>Your top offerings*</h1>
