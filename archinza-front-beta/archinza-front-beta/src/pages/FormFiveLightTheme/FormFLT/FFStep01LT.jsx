...
const FFStep01LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      f_services: Joi.array().min(1).required().label("Services").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { f_services: selectedOptions };
  ...
  const fetchData = async () => {
    ...
    if (data) {
      ...
      const updatedOption = data?.services?.map((option) => {
        ...
        if (BusinessAccountContext?.data?.f_services?.includes(option.value)) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.f_services);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((3 / 8) * 100);
    }
  }, [isActive]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="f_services_error">
          {formError?.f_services && (
            <p className="error">{formError?.f_services}</p>
          )}
        </div>
      </div>
