...
const FFStep02LT = ({
  ...
  const formALocationList = locations.map((option) => (
    <React.Fragment key={option}>
      <RadioButton
        ...
        checked={values.f_project_location === option}
  ...
  useEffect(() => {
    setValues({
      f_project_location: BusinessAccountContext?.data?.f_project_location,
    });
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((4 / 8) * 100);
    }
  }, [isActive]);
  useEffect(() => {
    fetchData();
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="f_project_location_error">
          <p className={`${style.top_error_with_space} ${style.error}`}>
            {formError.f_project_location}
          </p>
        </div>
      </div>
