...
const FFStep04LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      f_design_style: Joi.array().min(1).label("Design").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { f_design_style: selectedOptions };
    ...
    let data = await http.post(
      ...
      {
        f_design_style: selectedOptions,
        status: saveStatus,
      }
    );
  ...
  const fetchServices = async () => {
    ...
    if (data) {
      const updatedOption = data?.design_styles?.map((option) => {
        if (
          BusinessAccountContext?.data?.f_design_style?.includes(option.value)
        ) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.f_design_style);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((6 / 8) * 100);
    }
  }, [isActive]);
  useEffect(() => {
    fetchServices();
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="f_design_style_error">
          <p className="error">{formError?.f_design_style}</p>
        </div>
      </div>
