...
const FIStep04LT = ({
  ...
  const handleFileUpload = (uploadedFileData, stateType) => {
    ...
    if (state == "removed") {
      ...
      BusinessContext.update({
        ...
        i_documents_media: [...data, ...uploadedFileData],
      });
    } else {
      ...
      BusinessContext.update({
        ...BusinessContext.data,
        i_documents_media: [...media, ...uploadedFileData],
      });
    }
  };
  ...
  useEffect(() => {
    ...
    console.log(BusinessContext?.data?.i_documents_media);

    setMedia(BusinessContext?.data?.i_documents_media);
  }, [currentStep]);

  useEffect(() => {
    if (isActive) {
      progressStatus((6 / 6) * 100);
    }
  }, [isActive]);
  return (
    <>
      ...
      <div className={`${style.steps} ${style.fastep12}`}>
        ...
        <div className={style.customdUpload_wrapper}>
          <BusinessFileUpload
            ...
            documentMediaType={"i_documents_media"}
          />
