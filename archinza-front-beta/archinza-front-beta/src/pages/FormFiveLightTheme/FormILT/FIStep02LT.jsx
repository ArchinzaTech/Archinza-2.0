...
const FIStep02LT = ({
  ...
  const handleSubmit = async () => {
    if (date && date?.$d == "Invalid Date") {
      setFormError({
        i_establishment_year: "Invalid Date",
      });
      helper.scroll(
    ...
    values.i_establishment_year = newDate;
  ...
  useEffect(() => {
    setDate(
      BusinessAccountContext?.data?.i_establishment_year !== ""
        ? dayjs(BusinessAccountContext?.data?.i_establishment_year)
        : ""
    );
    setValues(() => ({
      i_establishment_year:
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="i_establishment_year_error">
          <p className={`${style.rstep02Error} ${style.error}`}>
            {formError.i_establishment_year}
          </p>
        </div>
      </div>
