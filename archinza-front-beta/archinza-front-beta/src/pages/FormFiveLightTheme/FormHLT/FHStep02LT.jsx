...
const FHStep02LT = ({
  ...
  const handleSubmit = async () => {
    if (date && date?.$d == "Invalid Date") {
      setFormError({
        h_establishment_year: "Invalid Date",
      });
      helper.scroll(
    ...
    values.h_establishment_year = newDate;
  ...
  useEffect(() => {
    setDate(
      BusinessAccountContext?.data?.h_establishment_year !== ""
        ? dayjs(BusinessAccountContext?.data?.h_establishment_year)
        : ""
    );
    setValues(() => ({
      h_establishment_year:
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="h_establishment_year_error">
          <p className={`${style.rstep02Error} ${style.error}`}>
            {formError.h_establishment_year}
          </p>
        </div>
      </div>
