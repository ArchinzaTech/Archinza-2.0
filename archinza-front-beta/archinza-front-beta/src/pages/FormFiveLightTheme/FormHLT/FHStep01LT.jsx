...
const FHStep01LT = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      h_services: Joi.array().min(1).required().label("Services").messages({
  ...
  const handleSubmit = async () => {
    let form_values = { h_services: selectedOptions };
  ...
  const fetchData = async () => {
    ...
    if (data) {
      ...
      const updatedOption = data?.services?.map((option) => {
        ...
        if (BusinessAccountContext?.data?.h_services?.includes(option.value)) {
  ...
  useEffect(() => {
    setSelectedOptions(BusinessAccountContext?.data?.h_services);
  }, [currentStep]);
  ...
  return (
    <>
      <div className={style.text_container}>
        ...
        <div id="h_services_error">
          {formError?.h_services && (
            <p className="error">{formError?.h_services}</p>
          )}
        </div>
      </div>
