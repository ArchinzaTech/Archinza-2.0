...
import "./aboutPopupV2.scss";
import Modal from "react-bootstrap/Modal";
import { blackClose, modalClose, modalCloseIcon } from "../../../../../images";
...
const AboutPopupV2 = (props) => {
  ...
  const schema = Joi.object({
    ...
    services: Joi.array().min(1).max(5).required().messages({
      "array.min": "Please select at least 1 and max 5 services",
      "array.max": "Please select at least 1 and max 5 services",
      "array.base": "Products/services selection is required",
    }),
  });
  ...
  useEffect(() => {
    ...
    if (props.data && props.data.services) {
      setInitialServices(props.data.featured_services || []);
      const services = props.data.services
        ?.map((it) => ({
          label: helper.capitalizeWords(it),
          value: helper.capitalizeWords(it),
        }))
  ...
  return (
    <>
      <Modal
        {...props}
        centered
        className={`Complete_actions_popup bussinessEdit_about_Modal_v2`}
        backdropClassName="custom-backdrop"
      >
        <Modal.Header>
          <button
            className="Complete_actions_popup_cancel_btn inside_cancel_btn--about"
            onClick={() => {
              setValidationErrors({});
              props.onHide();
            }}
        ...
        <Modal.Body>
            ...
            <>
              ...
              <div className="complete_actions_step_one_wrapper">
                ...
                {validationErrors.bio && (
                  <div className="error_text">{validationErrors.bio}</div>
                )}
              </div>
              <div className="form_autocomplete_b_edit_wrapper_popUp">
                <div className={`${style.steps} ${style.fastep04}`}>
                  <div
                    className={`${style.add_more_wrap} ${style.add_more_v2}`}
                  >
                    <div className={`${style.field_wrapper} field_wrapper`}>
                      <div className="autocomplete_coustom_wrapper mx-auto">
                        <button
                          type="button"
          ...
          {props.modalType === "products" && (
            <div className="form_autocomplete_b_edit_wrapper_popUp">
              <h2 className="automplete_b_edit_heading mt-0">
