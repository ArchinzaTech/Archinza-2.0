...
const BEditProfile02 = () => {
  ...
  const employeeCountList = sections[2].employeesRangeList.map((option) => (
    <RadioButton
      ...
      lightTheme
      label={option}
      labelId={"employee_count_bedit2" + option}
      name="employees_range_bedit2"
      checked={sections[2].tempSelectedOption === option}
      onChange={() => handleRadioChange(option, 2)}
    />
  ));

  const selectedProjectSizeList = sections[5].tempSelectedOptions?.length ? (
    sections[5].tempSelectedOptions?.map((option) => (
      <li className="selectedValuePill">{option}</li>
    ))
  ) : (
    <li className="selectedValuePill">No Options Selected</li>
  );

  const selectedProjectScopeList = sections[10].tempSelectedOptions?.length ? (
    sections[10].tempSelectedOptions.map((option) => (
      <li className="selectedValuePill">{option}</li>
    ))
  ) : (
    <li className="selectedValuePill">No Options Selected</li>
  ...
  const projectSizeList = sections[5].globalData.map((option) => (
    <CheckboxButton
      ...
      lightTheme
      label={option}
      labelId={"project_size_bedit2" + option}
  ...
  const projectScopeList = sections[10].globalData.map((option) => (
    <CheckboxButton
      ...
      lightTheme
      label={option}
      labelId={"project_scope_bedit2" + option}
  ...
  const locationList = sections[6].locationList.map((option) => (
    <RadioButton
      ...
      lightTheme
      label={option}
      labelId={"location_bedit2" + option}
      name="project_location_bedit2"
  ...
  const budgetList = sections[9].projectBudgetsList
    ...
    .map((option) => (
      <RadioButton
        ...
        lightTheme
        label={option}
        labelId={"budgetList_bedit2" + option}
        name="budget_list_bedit2"
  ...
  const handleDeleteProjectsMedia = async (
    ...
    let payload;
    if (sectionId === 14) {
      payload = { b_completed_projects_media: filteredMedia };
    } else if (sectionId === 15) {
      payload = { b_project_renders_media: filteredMedia };
    } else if (sectionId === 16) {
      payload = { b_work_in_progress_site_media: filteredMedia };
    } else if (sectionId === 17) {
      payload = { b_other_media: filteredMedia };
    } else if (sectionId === 18) {
      ...
      payload = { b_workspace_category_media: updatedTempWorkSpaceData };
  ...
  const handleWorkspaceCategoryChange = async (option) => {
    const filteredCategoryData =
      BusinessContext?.data?.b_workspace_category_media?.filter(
  ...
  const updateMedia = async (mediaId, visibility, sectionId, mediaType) => {
    ...
    let payload;
    if (sectionId === 14) {
      payload = { b_completed_projects_media: updatedMedia };
    } else if (sectionId === 15) {
      payload = { b_project_renders_media: updatedMedia };
    } else if (sectionId === 16) {
      payload = { b_work_in_progress_site_media: updatedMedia };
    } else if (sectionId === 17) {
      payload = { b_other_media: updatedMedia };
    } else if (sectionId === 18) {
      ...
      payload = { b_workspace_category_media: tempWorkSpaceData };
    }
    ...
    setSections((prevSections) => ({
      ...
      [sectionId]: {
        ...
        tempWorkSpaceData:
          sectionId === 18
            ? payload.b_workspace_category_media
  ...
  const validation = (sectionId, formData) => {
    const schemaValidation = {
      0: Joi.object({
        b_services: Joi.array()
          .min(1)
          .required()
          .messages({ "array.min": "Select atleast One Service" }),
      }),
      1: Joi.object({
        b_additional_services: Joi.array()
          .min(1)
          .required()
          .messages({ "array.min": "Select atleast One Service" }),
      }),
      ...
      3: Joi.object({
        b_establishment_year: Joi.string()
          .required()
  ...
  const handleUpdateMedia = (data, section) => {
    switch (section) {
      ...
      case "Completed Project Photos/Videos":
        ...
        BusinessContext.update({
          ...BusinessContext.data,
          b_completed_projects_media: data,
        });
        break;
      ...
      case "Work In Progress":
        ...
        BusinessContext.update({
          ...BusinessContext.data,
          b_work_in_progress_site_media: data,
        });
        break;
      case "other":
        setSections((prevSections) => ({
      ...
      case "workspace":
        ...
        BusinessContext.update({
          ...BusinessContext.data,
          b_workspace_category_media: data,
        });
        break;
  ...
  const handleCurrencyChange = async (sectionId, e) => {
    ...
    let querySection = sectionId == 9 ? "budget" : "min_fees";
    const originalSelectedOption = sections[sectionId].selectedOption || "";
    if (e.target.value !== "INR") {
      const data = await http.get(
        config.api_url +
          `/business/get-currency?base=INR&target=${e.target.value}&section=${querySection}&business_type=business_type_b`
      );
      if (data) {
        setSections((prevSections) => ({
          ...prevSections,
          [sectionId]: {
            ...
            tempSelectedOption: sections[sectionId].selectedOption,
    ...
    setSections((prevSections) => ({
      ...
      [sectionId]: {
        ...prevSections[sectionId],
        tempSelectedCurrency: e.target.value,
        tempSelectedOption: originalSelectedOption,
        [sectionType]: GlobalDataContext?.business_type_b[
          sectionId == 9 ? "budget" : "min_fees"
        ].map((it) => it[sectionId == 9 ? "type" : "fee"]),
      },
    }));
  };
  ...
  const validateSelectedDate = (date) => {
    if (!date) {
      setSections((prevSections) => ({
        ...
        3: {
          ...prevSections[3],
          errors: { b_establishment_year: "Invalid Date" },
        },
      }));
      return false;
    }
    ...
    if (date?.$y > new Date().getFullYear() || date?.$y < currentYear - 50) {
      setSections((prevSections) => ({
        ...
        3: {
          ...prevSections[3],
          errors: {
            b_establishment_year: "Date must be within the last 50 years",
          },
        },
      }));
      return false;
  ...
  const handleSubmit = async (sectionId) => {
    ...
    switch (sectionId) {
      case 0:
        formData["b_services"] = sections[0].selectedOptions;
        ...
        break;
      case 1:
        formData["b_additional_services"] = sections[1].selectedOptions;
        ...
        break;
      case 2:
        formData["b_employees_range"] = sections[2].tempSelectedOption;
      ...
      case 3:
        ...
        if (!validDate) {
          ...
          formData["b_establishment_year"] = validDate?.toString();
      ...
      case 5:
        ...
        formData["b_project_sizes"] = project_sizes;
        ...
        break;
      case 6:
        formData["b_project_location"] = sections[6].tempSelectedOption;
        ...
        break;
      case 7:
        formData["b_project_typology"] = sections[7].selectedOptions;
        ...
        break;
      case 8:
        formData["b_design_style"] = sections[8].selectedOptions;
      ...
      case 10:
        ...
        formData["b_project_scope"] = options;
      ...
      case 11:
        formData["b_project_mimimal_fee"] = {
  ...
  const handleEditButton = async (sectionId) => {
    ...
    if (sectionId == 9 || sectionId == 11) {
      const querySection = sectionId == 9 ? "budget" : "min_fees";
      if (sections[sectionId].tempSelectedCurrency !== "INR") {
        const data = await http.get(
          config.api_url +
            `/business/get-currency?base=INR&target=${sections[sectionId].tempSelectedCurrency}&section=${querySection}&business_type=business_type_b`
        );
        if (data) {
          setSections((prevSections) => ({
            ...prevSections,
  ...
  useEffect(() => {
    if (GlobalDataContext) {
      const globalData = GlobalDataContext?.business_type_b;
      const updatedSections = { ...sections };
      ...
      updatedSections[5].globalData = [
        "ALL",
        ...(globalData.project_sizes || []),
      ];
      ...
      updatedSections[9].projectBudgetsList =
        globalData.budget
      ...
      updatedSections[11].projectFeeList =
        globalData.min_fees
          .filter((f) => f.currency === "INR")
          .map((it) => it.fee) || [];
  ...
  useEffect(() => {
    if (BusinessContext?.data) {
      ...
      const {
        b_services,
        b_additional_services,
        b_employees_range,
        b_establishment_year,
        b_project_sizes,
        b_project_location,
        b_project_typology,
        b_design_style,
        ...
        b_documents_media,
        b_firm_connect_data,
        b_completed_projects_media,
        b_project_renders_media,
        b_work_in_progress_site_media,
        b_other_media,
        b_workspace_category_media,
      } = BusinessContext.data;

      const parsedEstablishmentYear = b_establishment_year
        ? dayjs(`${b_establishment_year}-01-01`)
        : null;

      updatedSections[0].selectedOptions = b_services || [];
      updatedSections[0].tempSelectedOptions = b_services || [];

      updatedSections[1].selectedOptions = b_additional_services || [];
      updatedSections[1].tempSelectedOptions = b_additional_services || [];

      updatedSections[2].selectedOption = b_employees_range || "";
      updatedSections[2].tempSelectedOption = b_employees_range || "";

      updatedSections[3].selectedOption = parsedEstablishmentYear;
      updatedSections[3].tempSelectedOption = parsedEstablishmentYear;

      updatedSections[5].selectedOptions = b_project_sizes || [];
      updatedSections[5].tempSelectedOptions = b_project_sizes || [];
      ...
      updatedSections[6].selectedOption = b_project_location || "";
      updatedSections[6].tempSelectedOption = b_project_location || "";

      updatedSections[7].selectedOptions = b_project_typology || [];
      updatedSections[7].tempSelectedOptions = b_project_typology || [];

      updatedSections[8].selectedOptions = b_design_style || [];
      updatedSections[8].tempSelectedOptions = b_design_style || [];

      if (b_avg_project_budget) {
        updatedSections[9].selectedOption = b_avg_project_budget.budget || "";
        updatedSections[9].tempSelectedOption =
          b_avg_project_budget.budget || "";
        updatedSections[9].selectedCurrency =
          b_avg_project_budget.currency || "";
        updatedSections[9].tempSelectedCurrency =
          b_avg_project_budget.currency || "";
      }

      updatedSections[10].selectedOptions = b_project_scope || [];
      updatedSections[10].tempSelectedOptions = b_project_scope || [];
      // if (sections[10].globalData?.length) {
      //   updatedSections[10].allSelected =
      //     sections[10].globalData.slice(1).length === project_scope.length;
      // }
      if (b_project_mimimal_fee) {
        updatedSections[11].selectedOption = b_project_mimimal_fee.fee || "";
        updatedSections[11].tempSelectedOption =
          b_project_mimimal_fee.fee || "";
        updatedSections[11].selectedCurrency =
          b_project_mimimal_fee.currency || "";
        updatedSections[11].tempSelectedCurrency =
          b_project_mimimal_fee.currency || "";
      }

      updatedSections[4].websiteLink = b_firm_connect_data.website_link || "";
      updatedSections[4].linkedinLink = b_firm_connect_data.linkedin_link || "";
      updatedSections[4].instagramLink =
        b_firm_connect_data.instagram_handle || "";
      updatedSections[4].behanceLink = b_firm_connect_data.behance_link || "";
      updatedSections[4].other = b_firm_connect_data.other || "";
      updatedSections[4].gstNumber = b_firm_connect_data.gst_number || "";
      updatedSections[4].tempValues = {
        websiteLink: b_firm_connect_data.website_link || "",
        linkedinLink: b_firm_connect_data.linkedin_link || "",
        instagramLink: b_firm_connect_data.instagram_handle || "",
        behanceLink: b_firm_connect_data.behance_link || "",
        other: b_firm_connect_data.other || "",
        gstNumber: b_firm_connect_data.gst_number || "",
      };
      updatedSections[13].documentsMedia = b_documents_media || [];
      updatedSections[14].completedProjectsMedia =
        b_completed_projects_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      updatedSections[15].projectRendersMedia =
        b_project_renders_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      updatedSections[16].workInProgressMedia =
        b_work_in_progress_site_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      updatedSections[17].otherMedia =
        b_other_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      ...
      updatedSections[18].workSpaceData =
        b_workspace_category_media?.filter(
          (it) => it.category === selectedCategory
        ) || [];
      updatedSections[18].tempWorkSpaceData = b_workspace_category_media || [];
      setSections(updatedSections);
    }
  }, [BusinessContext?.data]);
          ...
          <div className="firm_project_container">
            ...
            >
              ...
              {Object.keys(sections[0].errors).length > 0 && (
                <p className="error">{sections[0].errors.b_services}</p>
              )}
              {activeQuestion === 0 && (
            ...
            >
              ...
              {Object.keys(sections[1].errors).length > 0 && (
                <p className="error">
                  {sections[1].errors.b_additional_services}
                </p>
              )}
              {activeQuestion === 1 && (
            ...
            >
              ...
              {Object.keys(sections[2].errors).length > 0 && (
                <p className="error">{sections[2].errors.b_employees_range}</p>
              )}
              {activeQuestion === 2 && (
                <div className="save_cancel_cta no_space">
              ...
              <div className="radio">
                {activeQuestion !== 3 ? (
                  ...
                  <div className="year_picker_wrapper">
                    ...
                    />
                    <p className="error">
                      {sections[3].errors.b_establishment_year}
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="firm_project_container">
            ...
            <div
              className={`brand_subsec first_box with_checkboxes ${
                activeQuestion === 5 ? "" : activeQuestion !== null && "disable"
              }`}
            >
              <div className="title_pin">
              ...
              {Object.keys(sections[5].errors).length > 0 && (
                <p className="error">{sections[5].errors.b_project_sizes}</p>
              )}
              {activeQuestion === 5 && (
                <div className="save_cancel_cta no_space">
                  <button className="save_cta" onClick={() => handleSubmit(5)}>
                    Save
                  </button>
                  <button
                    className="cancel_cta"
            ...
            >
              ...
              {Object.keys(sections[6].errors).length > 0 && (
                <p className="error">{sections[6].errors.b_project_location}</p>
              )}
              {activeQuestion === 6 && (
                <div className="save_cancel_cta no_space">
            ...
            >
              ...
              {Object.keys(sections[7].errors).length > 0 && (
                <p className="error">{sections[7].errors.b_project_typology}</p>
              )}
              {activeQuestion === 7 && (
                <div className="save_cancel_cta">
            ...
            >
              ...
              {Object.keys(sections[8].errors).length > 0 && (
                <p className="error">{sections[8].errors.b_design_style}</p>
              )}
              {activeQuestion === 8 && (
                <div className="save_cancel_cta">
            ...
            <div
              className={`brand_subsec ${
                activeQuestion === 9 ? "" : activeQuestion !== null && "disable"
              }`}
            >
              <div className="title_pin">
              ...
              <div className="radio">
                {activeQuestion !== 9 ? (
                  <ul className="radio" style={{ pointerEvents: "none" }}>
                    ...
                    <li className="selectedValuePill">
                      {sections[9].tempSelectedCurrency ||
              ...
              {Object.keys(sections[9].errors).length > 0 && (
                <p className="error">
                  {sections[9].errors.b_avg_project_budget}
                </p>
              )}
              {activeQuestion === 9 && (
                <div className="save_cancel_cta">
                  <button className="save_cta" onClick={() => handleSubmit(9)}>
                    Save
                  </button>
                  <button
                    className="cancel_cta"
            ...
            >
              ...
              {activeQuestion === 10 && (
                <div className="save_cancel_cta">
                  <button className="save_cta" onClick={() => handleSubmit(10)}>
                    Save
                  </button>
                  <button
                    className="cancel_cta"
            ...
            >
              ...
              <div className="radio">
                {activeQuestion !== 11 ? (
                  <ul className="radio" style={{ pointerEvents: "none" }}>
                    ...
                    <li className="selectedValuePill">
                      {sections[11].tempSelectedCurrency ||
                        "No Currency Selected"}
                    </li>
                  </ul>
                ) : (
              ...
              {Object.keys(sections[11].errors).length > 0 && (
                <p className="error">
                  {sections[11].errors.b_project_mimimal_fee}
                </p>
              )}
              {activeQuestion === 11 && (
                <div className="save_cancel_cta">
                  <button className="save_cta" onClick={() => handleSubmit(11)}>
                    Save
                  </button>
                  <button
                    className="cancel_cta"
              ...
              <div className="col-lg-6 p-0 border_rightt">
                <div className="profile_brochure_wrapper section1">
                  ...
                  <p className="notice_msg">
                    Maximum File Size Is 5MB. PDF, PPT
                  </p>
