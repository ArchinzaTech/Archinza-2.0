...
const BEditProfile07 = () => {
  ...
  const handleDeleteProjectsMedia = async (
    ...
    let payload;
    if (sectionId === 14) {
      payload = { g_completed_projects_media: filteredMedia };
    } else if (sectionId === 15) {
      payload = { g_project_renders_media: filteredMedia };
    } else if (sectionId === 16) {
      payload = { g_work_in_progress_site_media: filteredMedia };
    } else if (sectionId === 17) {
      payload = { g_other_media: filteredMedia };
    } else if (sectionId === 18) {
      ...
      payload = { g_workspace_category_media: updatedTempWorkSpaceData };
  ...
  const handleWorkspaceCategoryChange = async (option) => {
    const filteredCategoryData =
      BusinessContext?.data?.g_workspace_category_media?.filter(
  ...
  const updateMedia = async (mediaId, visibility, sectionId, mediaType) => {
    ...
    let payload;
    if (sectionId === 14) {
      payload = { g_completed_projects_media: updatedMedia };
    } else if (sectionId === 15) {
      payload = { g_project_renders_media: updatedMedia };
    } else if (sectionId === 16) {
      payload = { g_work_in_progress_site_media: updatedMedia };
    } else if (sectionId === 17) {
      payload = { g_other_media: updatedMedia };
    } else if (sectionId === 18) {
      ...
      payload = { g_workspace_category_media: tempWorkSpaceData };
    }
    ...
    setSections((prevSections) => ({
      ...
      [sectionId]: {
        ...
        tempWorkSpaceData:
          sectionId === 18
            ? payload.g_workspace_category_media
  ...
  const validation = (sectionId, formData) => {
    const schemaValidation = {
      0: Joi.object({
        g_services: Joi.array()
          .min(1)
          .required()
          .messages({ "array.min": "Select atleast One Service" }),
      }),
      3: Joi.object({
        g_establishment_year: Joi.string()
          .required()
  ...
  const handleUpdateMedia = (data, section) => {
    switch (section) {
      ...
      case "Completed Project Photos/Videos":
        ...
        BusinessContext.update({
          ...BusinessContext.data,
          g_completed_projects_media: data,
        });
        break;
      ...
      case "Work In Progress":
        ...
        BusinessContext.update({
          ...BusinessContext.data,
          g_work_in_progress_site_media: data,
        });
        break;
      case "other":
        setSections((prevSections) => ({
      ...
      case "workspace":
        ...
        BusinessContext.update({
          ...BusinessContext.data,
          g_workspace_category_media: data,
        });
        break;
  ...
  const validateSelectedDate = (date) => {
    if (!date) {
      setSections((prevSections) => ({
        ...
        3: {
          ...prevSections[3],
          errors: { g_establishment_year: "Invalid Date" },
        },
      }));
      return false;
    }
    ...
    if (date?.$y > new Date().getFullYear() || date?.$y < currentYear - 50) {
      setSections((prevSections) => ({
        ...
        3: {
          ...prevSections[3],
          errors: {
            g_establishment_year: "Date must be within the last 50 years",
          },
        },
      }));
      return false;
  ...
  const handleSubmit = async (sectionId) => {
    ...
    switch (sectionId) {
      case 0:
        formData["g_services"] = sections[0].selectedOptions;
      ...
      case 3:
        ...
        if (!validDate) {
          ...
          formData["g_establishment_year"] = validDate?.toString();
  ...
  useEffect(() => {
    if (GlobalDataContext) {
      const globalData = GlobalDataContext?.business_type_g;
  ...
  useEffect(() => {
    if (BusinessContext?.data) {
      ...
      const {
        g_services,
        g_establishment_year,
        g_documents_media,
        g_firm_connect_data,
        g_completed_projects_media,
        g_project_renders_media,
        g_work_in_progress_site_media,
        g_other_media,
        g_workspace_category_media,
      } = BusinessContext.data;

      const parsedEstablishmentYear = g_establishment_year
        ? dayjs(`${g_establishment_year}-01-01`)
        : null;

      updatedSections[0].selectedOptions = g_services || [];
      updatedSections[0].tempSelectedOptions = g_services || [];

      updatedSections[3].selectedOption = parsedEstablishmentYear;
      updatedSections[3].tempSelectedOption = parsedEstablishmentYear;

      updatedSections[4].websiteLink = g_firm_connect_data.website_link || "";
      updatedSections[4].linkedinLink = g_firm_connect_data.linkedin_link || "";
      updatedSections[4].instagramLink =
        g_firm_connect_data.instagram_handle || "";
      updatedSections[4].behanceLink = g_firm_connect_data.behance_link || "";
      updatedSections[4].other = g_firm_connect_data.other || "";
      updatedSections[4].gstNumber = g_firm_connect_data.gst_number || "";
      updatedSections[4].tempValues = {
        websiteLink: g_firm_connect_data.website_link || "",
        linkedinLink: g_firm_connect_data.linkedin_link || "",
        instagramLink: g_firm_connect_data.instagram_handle || "",
        behanceLink: g_firm_connect_data.behance_link || "",
        other: g_firm_connect_data.other || "",
        gstNumber: g_firm_connect_data.gst_number || "",
      };
      updatedSections[13].documentsMedia = g_documents_media || [];
      updatedSections[14].completedProjectsMedia =
        g_completed_projects_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      updatedSections[15].projectRendersMedia =
        g_project_renders_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      updatedSections[16].workInProgressMedia =
        g_work_in_progress_site_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      updatedSections[17].otherMedia =
        g_other_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      ...
      updatedSections[18].workSpaceData =
        g_workspace_category_media?.filter(
          (it) => it.category === selectedCategory
        ) || [];
      updatedSections[18].tempWorkSpaceData = g_workspace_category_media || [];
      setSections(updatedSections);
    }
  }, [BusinessContext?.data]);
          ...
          <div className="firm_project_container">
            ...
            >
              ...
              {Object.keys(sections[0].errors).length > 0 && (
                <p className="error">{sections[0].errors.g_services}</p>
              )}
              {activeQuestion === 0 && (
              ...
              <div className="radio">
                {activeQuestion !== 3 ? (
                  ...
                  <div className="year_picker_wrapper">
                    ...
                    />
                    <p className="error">
                      {sections[3].errors.g_establishment_year}
                    </p>
                  </div>
                )}
              </div>
