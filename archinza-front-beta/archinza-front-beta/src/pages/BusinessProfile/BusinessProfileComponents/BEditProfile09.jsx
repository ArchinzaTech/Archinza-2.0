...
const BEditProfile09 = () => {
  ...
  const handleDeleteProjectsMedia = async (
    ...
    let payload;
    if (sectionId === 14) {
      payload = { i_completed_projects_media: filteredMedia };
    } else if (sectionId === 15) {
      payload = { i_project_renders_media: filteredMedia };
    } else if (sectionId === 16) {
      payload = { i_work_in_progress_site_media: filteredMedia };
    } else if (sectionId === 17) {
      payload = { i_other_media: filteredMedia };
    } else if (sectionId === 18) {
      ...
      payload = { i_workspace_category_media: updatedTempWorkSpaceData };
  ...
  const handleWorkspaceCategoryChange = async (option) => {
    const filteredCategoryData =
      BusinessContext?.data?.i_workspace_category_media?.filter(
  ...
  const updateMedia = async (mediaId, visibility, sectionId, mediaType) => {
    ...
    let payload;
    if (sectionId === 14) {
      payload = { i_completed_projects_media: updatedMedia };
    } else if (sectionId === 15) {
      payload = { i_project_renders_media: updatedMedia };
    } else if (sectionId === 16) {
      payload = { i_work_in_progress_site_media: updatedMedia };
    } else if (sectionId === 17) {
      payload = { i_other_media: updatedMedia };
    } else if (sectionId === 18) {
      ...
      payload = { i_workspace_category_media: tempWorkSpaceData };
    }
    ...
    setSections((prevSections) => ({
      ...
      [sectionId]: {
        ...
        tempWorkSpaceData:
          sectionId === 18
            ? payload.i_workspace_category_media
  ...
  const validation = (sectionId, formData) => {
    const schemaValidation = {
      ...
      3: Joi.object({
        i_establishment_year: Joi.string()
          .required()
  ...
  const handleUpdateMedia = (data, section) => {
    switch (section) {
      ...
      case "Completed Project Photos/Videos":
        ...
        BusinessContext.update({
          ...BusinessContext.data,
          i_completed_projects_media: data,
        });
        break;
      ...
      case "Work In Progress":
        ...
        BusinessContext.update({
          ...BusinessContext.data,
          i_work_in_progress_site_media: data,
        });
        break;
      case "other":
        setSections((prevSections) => ({
      ...
      case "workspace":
        ...
        BusinessContext.update({
          ...BusinessContext.data,
          i_workspace_category_media: data,
        });
        break;
  ...
  const validateSelectedDate = (date) => {
    if (!date) {
      setSections((prevSections) => ({
        ...
        3: {
          ...prevSections[3],
          errors: { i_establishment_year: "Invalid Date" },
        },
      }));
      return false;
    }
    ...
    if (date?.$y > new Date().getFullYear() || date?.$y < currentYear - 50) {
      setSections((prevSections) => ({
        ...
        3: {
          ...prevSections[3],
          errors: {
            i_establishment_year: "Date must be within the last 50 years",
          },
        },
      }));
      return false;
  ...
  const handleSubmit = async (sectionId) => {
    ...
    switch (sectionId) {
      case 0:
        formData["brand_name"] = sections[0].tempBrandName;
        ...
        setSections((prevSections) => ({
          ...
          0: {
            ...prevSections[0],
            brandName: prevSections[0].tempBrandName,
          },
        }));
        break;
      case 3:
        ...
        if (!validDate) {
          ...
          formData["i_establishment_year"] = validDate?.toString();
    ...
    //   i_establishment_year: validDate,
  ...
  const handleCancel = async (sectionId) => {
    ...
    if (sectionId == 0) {
      setSections((prevSections) => ({
        ...
        [sectionId]: {
          ...prevSections[sectionId],
          tempBrandName: prevSections[sectionId].brandName,
          errors: {},
        },
      }));
      activeQuestionHandler(sectionId);
  ...
  useEffect(() => {
    if (BusinessContext?.data) {
      ...
      const {
        brand_name,
        i_establishment_year,
        i_documents_media,
        i_firm_connect_data,
        i_completed_projects_media,
        i_project_renders_media,
        i_work_in_progress_site_media,
        i_other_media,
        i_workspace_category_media,
      } = BusinessContext.data;

      const parsedEstablishmentYear = i_establishment_year
        ? dayjs(`${i_establishment_year}-01-01`)
        : null;

      updatedSections[0].brandName = brand_name || [];
      updatedSections[0].tempBrandName = brand_name || [];

      updatedSections[3].selectedOption = parsedEstablishmentYear;
      updatedSections[3].tempSelectedOption = parsedEstablishmentYear;

      updatedSections[4].websiteLink = i_firm_connect_data.website_link || "";
      updatedSections[4].linkedinLink = i_firm_connect_data.linkedin_link || "";
      updatedSections[4].instagramLink =
        i_firm_connect_data.instagram_handle || "";
      updatedSections[4].behanceLink = i_firm_connect_data.behance_link || "";
      updatedSections[4].other = i_firm_connect_data.other || "";
      updatedSections[4].gstNumber = i_firm_connect_data.gst_number || "";
      updatedSections[4].tempValues = {
        websiteLink: i_firm_connect_data.website_link || "",
        linkedinLink: i_firm_connect_data.linkedin_link || "",
        instagramLink: i_firm_connect_data.instagram_handle || "",
        behanceLink: i_firm_connect_data.behance_link || "",
        other: i_firm_connect_data.other || "",
        gstNumber: i_firm_connect_data.gst_number || "",
      };
      updatedSections[13].documentsMedia = i_documents_media || [];
      updatedSections[14].completedProjectsMedia =
        i_completed_projects_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      updatedSections[15].projectRendersMedia =
        i_project_renders_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      updatedSections[16].workInProgressMedia =
        i_work_in_progress_site_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      updatedSections[17].otherMedia =
        i_other_media?.map((media) => ({
          ...media,
          visibility: media.visibility || false,
        })) || [];
      ...
      updatedSections[18].workSpaceData =
        i_workspace_category_media?.filter(
          (it) => it.category === selectedCategory
        ) || [];
      updatedSections[18].tempWorkSpaceData = i_workspace_category_media || [];
      setSections(updatedSections);
    }
  }, [BusinessContext?.data]);
  ...
  return (
    <div className="row g-0">
          ...
          <div className="firm_project_container">
            ...
            >
              <div className="title_pin">
                <h4 className="title">Name of your Brand*</h4>
                <img
                  src={editicon}
                  alt=""
                  className="edit_icon"
              ...
              {activeQuestion === 0 ? (
                ...
                <h4 className="title">{sections[0].brandName}</h4>
              )}
              {/* <BEAutocompleteOthers
                allOptionData={businessProfileData[1].data.servicesDropdownArr}
                selectedData={businessProfileData[0].data.selectedServicesData}
              ...
              {activeQuestion === 0 && (
                <>
                  <FullWidthTextField
                    ...
                    labelId="business_name_bedit9"
                    value={sections[0].tempBrandName}
                    onChange={handleBrandChange}
                  />
                  <p className="error">{sections[0].errors.brand_name}</p>
            ...
            >
              <div className="title_pin">
                ...
                <img
                  ...
                  alt=""
                  className="edit_icon"
                  onClick={() => activeQuestionHandler(2)}
                />
              </div>
              <div>
                {activeQuestion !== 2 ? (
                  <ul className="radio" style={{ pointerEvents: "none" }}>
                    <li className="selectedValuePill">
                      {businessProfileData[1].data.selectedEmployeeCountArr[0]}
                    </li>
                  </ul>
                ) : (
              ...
              <div className="radio">
                {activeQuestion !== 3 ? (
                  ...
                  <div className="year_picker_wrapper">
                    ...
                    />
                    <p className="error">
                      {sections[3].errors.i_establishment_year}
                    </p>
                  </div>
                )}
              </div>
      ...
      {/* ABOUT PROJECTS START HERE */}
      {/* ABOUT PROJECTS START HERE */}
      <div className="col-lg-6 p-0" style={{ display: "none" }}>
        <div className="business_details_wrapper section2">
          <div className="firm_project_container">
            ...
            >
              <div className="title_pin">
                <h4 className="title">Your Current Minimum Project Fee?</h4>
                <img
                  src={editicon}
                  alt=""
                  className="edit_icon"
                  onClick={() => activeQuestionHandler(11)}
                />
              </div>
              <div className="radio">
                {activeQuestion !== 11 ? (
                  <ul className="radio" style={{ pointerEvents: "none" }}>
                    <li className="selectedValuePill">
                      {businessProfileData[1].data.selectedMinFeeData[0]}
                    </li>
                  </ul>
                ) : (
