...
const STStep03 = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      st_unmet_needs: Joi.string().required().messages({
  ...
  const handleSubmit = async (e) => {
    ...
    if (form_values.st_unmet_needs?.toLowerCase().includes("all")) {
      //insert all the options except "All of the above" in all_st_unmet_needs
      //insert only "All of the above" option in st_unmet_needs
      form_values.st_unmet_needs = options.find((option) =>
        option.value.toLowerCase().includes("all")
      )?.value;
      form_values.all_st_unmet_needs = options
  ...
  const concernList = options?.map((option, i) => (
    <React.Fragment key={`student-03-${i}`}>
      {/* <CheckboxButton label={option} labelId={`student-03-${option}`} /> */}
      <RadioButton
        extraSpace={true}
        label={option.label}
        labelId={`student-03-${i}`}
        name="st_unmet_needs"
        value={option.value}
        checked={values.st_unmet_needs === option.value}
  ...
  useEffect(() => {
    fetchOptions("st_unmet_needs");
  }, []);

  useEffect(() => {
    setValues({
      st_unmet_needs: ProAccess?.data?.st_unmet_needs,
    });
  }, [currentStep]);
      ...
      <div className={style.text_container}>
        ...
        <p
          className={`${style.description} ${
            formError?.st_unmet_needs && style.active
          }`}
        >
          Choose one
        </p>
        {formError?.st_unmet_needs && (
          <p className={`error ${style.error}`}>{formError?.st_unmet_needs}</p>
        )}
      </div>
