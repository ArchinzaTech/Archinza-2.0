...
const BOStep02 = ({
  ...
  const validate = async (data) => {
    let schemaObj = {
      bo_unmet_needs: Joi.string().required().label("Unmet needs"),
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (form_values.bo_unmet_needs?.toLowerCase().includes("all")) {
      //insert all the options except "All of the above" in all_bo_unmet_needs
      //insert only "All of the above" option in bo_unmet_needs
      form_values.bo_unmet_needs = options.find((option) =>
        option.value.toLowerCase().includes("all")
      )?.value;
      form_values.all_bo_unmet_needs = options
    ...
    <>
      <div className={style.text_container}>
        ...
        <h1 className={style.title}>
          What is your largest concern or unmet need at your work /
          business / firm?
          {/* <span className={style.owner_name}>
            {auth?.user?.name?.split(" ")[0]}
          </span>
          , as a Business/Firm Owner {width > 600 && <br />}what is your largest
          concern/unmet need... */}
        </h1>
        <p
